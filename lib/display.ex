defmodule Tbm.Display do
  ...
  def init(opts) do
    {
      :ok,
      %{
        stop_area: Keyword.get(opts, :stop_area),
  ...
  def handle_continue(
    :update_timetable,
    %{stop_area: %Tbm.StopArea{}, direction: %Tbm.Direction{}} = state
  ) do
    state =
      %{
        state |
        timetable: Tbm.Api.get_timetable(state.stop_area, state.direction),
      }
  ...
  @impl true
  def handle_cast({:set_direction, stop_area, direction}, state) do
    {
      :noreply,
      %{
        state |
  ...
  defp update_canvas(%{
    timetable: %Tbm.Timetable{nextDepartures: [next_departure | _] = next_departures},
    display: display,
  } = state) do
    ...
    state =
      state
      |> draw_route_name(next_departure)
      |> draw_logo(next_departure)
  ...
  defp draw_route_name(%{display: display} = state, next_departure) do
    ...
    # route name
    route_name = format_route_name(next_departure.route.name)
  ...
  defp draw_logo(%{display: display} = state, next_departure) do
    radius = div(@logo_diameter, 2)
    white = Tbm.Color.white() |> Tbm.Color.to_map()
    circle_color =
      next_departure.line.style.color
    ...
    RpiRgb.draw_text(
      ...
      white.r, white.g, white.b, ~c"#{next_departure.line.code}", 0
    )
...
# [stop_area] = Tbm.Api.search_stop_area("Parc de Musson"); [direction|_] = Tbm.Api.get_directions(stop_area); GenServer.whereis(Tbm.Display) |> GenServer.cast({:set_direction, stop_area, direction})
